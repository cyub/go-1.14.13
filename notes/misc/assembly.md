...
`power`函数中先通过`pushl %ebp`保存旧基址指针，即`_start`函数的开始地址，然后`movl %esp, %ebp`将esp保存的栈顶指针地址保存到ebp中，之后可以就可以通过ebp可以访问power的栈信息。至此`power`完成函数执行的准备过程。


为什么通过ebp访问power的栈信息，而不是通过esp反问栈信息？

这是因为在power生命周期内，ebp是不变的，总是`指向power栈帧开始地址 - 4`位置处。而esp总是随着栈的伸缩而变化。

`power`函数核心逻辑完成之后，需要完成返回过程。首先执行`movl %ebp, %esp`，恢复栈指针，然后执行`popl %ebp`，将堆栈中存储的caller的基址弹出，并将弹出的基址保存到ebp寄存器中。最后调用ret指令，交出控制权给caller，该指令相当于`popl %eip`。

`power`函数的准备过程以及返回过程是函数调用过程通用逻辑。
