...
1. 基于 `x86` 架构处理器的汇编语言，按照汇编语法风格，`x86`架构下汇编语言又可以分细为：
...
3. 存储器将相应的地址单元中的数据通过数据线送入 `CPU` 或 `CPU`通过数据线将数据送入相应的内存单元
...
在32位系统中进程空间(即用户空间）范围为 `0x00000000 ~ 0xbfffffff`，内核空间范围为 `0xc0000000 ~ 0xffffffff`。包括内核空间的详细内存布局，如下图：
...
寻址方式用于描述程序在指令执行过程中如何确定操作数（数据或内存地址）的地址。简单来说，寻址方式决定了指令如何找到操作数。
...
`test` 指令会对操作数进行按位与操作，并根据计算结果设置相应的标志寄存器：
...
**注意**：`CALL` 指令和 `RET` 指令是配对出现的。**`CALL`指令将返回地址压入堆栈，再把被调用过程的地址复制到指令指针寄存器。当过程准备返回时，它的`RET`指令从堆栈把返回地址弹回到指令指针寄存器**
